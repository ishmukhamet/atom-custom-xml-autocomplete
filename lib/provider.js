...
module.exports = {
  ...
  getAllAttributeNameCompletions: function(arg) {
    ...
    if (!this.completions[tag]) {
      return [];
    }

    ref = this.completions[tag].properties;
  ...
  getAttributeValueCompletion: function(arg) {
    ...
    if (!this.completions[tag].properties[attribute]) {
      return [];
    }

    ref = this.completions[tag].properties[attribute];
  ...
  loadCompletions: function() {
    ...
    // get path of current project if something's open
    var ref, projectFolder, source;
    if (atom.workspace.getActiveTextEditor()) {
      ref = atom.project.relativizePath(atom.workspace.getActiveTextEditor().getPath());
      projectFolder = ref[0];
    }

    // try to load manifest from ecosoft-lexema8 module
    if (atom.workspace.getActiveTextEditor() && fs.existsSync(path.resolve(projectFolder, 'node_modules/ecosoft-lexema8/source', 'controls.manifest.json'))) {
      return fs.readFile(path.resolve(projectFolder, 'node_modules/ecosoft-lexema8/source', 'controls.manifest.json'), (function(_this) {
    ...
    } else { // if something's wrong, load default
      return fs.readFile(path.resolve(__dirname, '..', 'controls.manifest.json'), (function(_this) {
        return function(error, content) {
          if (error == null) {
            _this.completions = JSON.parse(content);
          }
